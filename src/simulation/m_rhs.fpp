...
module m_rhs
    ...
    #:if MFC_CASE_OPTIMIZATION and mhd
    ...
    #:if MFC_CASE_OPTIMIZATION and mhd
        logical :: igr_mhd_debug_enabled = .false.
        logical :: igr_mhd_debug_first_order = .false.
        logical :: igr_mhd_debug_face_reported = .false.
        logical :: mhd_admissibility_check_enabled = .false.
        integer :: igr_mhd_debug_start = 0
        integer :: igr_mhd_debug_stop = 0
        integer :: igr_mhd_debug_budget_step = -1
        integer :: igr_mhd_debug_target_j = 0
        integer :: igr_mhd_debug_target_k = 0
        integer :: igr_mhd_debug_target_l = 0
    #:endif
...
contains
    ...
    impure subroutine s_initialize_rhs_module
        ...
        #:if MFC_CASE_OPTIMIZATION and mhd
            call s_igr_mhd_initialize_debug
        #:endif

    end subroutine s_initialize_rhs_module

    #:if MFC_CASE_OPTIMIZATION and mhd
        ! Local experimental controls. Environment variables avoid exposing
        ! diagnostic-only switches through MFC's public case-parameter schema.
        subroutine s_igr_mhd_initialize_debug

            call s_igr_mhd_read_env_logical('MFC_IGR_MHD_DEBUG', igr_mhd_debug_enabled)
            call s_igr_mhd_read_env_logical('MFC_IGR_MHD_FIRST_ORDER', igr_mhd_debug_first_order)
            call s_igr_mhd_read_env_logical('MFC_MHD_ADMISSIBILITY_CHECK', mhd_admissibility_check_enabled)
            call s_igr_mhd_read_env_integer('MFC_IGR_MHD_DEBUG_START', igr_mhd_debug_start)
            call s_igr_mhd_read_env_integer('MFC_IGR_MHD_DEBUG_STOP', igr_mhd_debug_stop)
            call s_igr_mhd_read_env_integer('MFC_IGR_MHD_DEBUG_BUDGET_STEP', igr_mhd_debug_budget_step)
            call s_igr_mhd_read_env_integer('MFC_IGR_MHD_DEBUG_TARGET_J', igr_mhd_debug_target_j)
            call s_igr_mhd_read_env_integer('MFC_IGR_MHD_DEBUG_TARGET_K', igr_mhd_debug_target_k)
            call s_igr_mhd_read_env_integer('MFC_IGR_MHD_DEBUG_TARGET_L', igr_mhd_debug_target_l)
        ...
        subroutine s_igr_mhd_read_env_integer(name, value)
            ...
            call get_environment_variable(name, text, status=status)
            if (status /= 0) return
            read (text, *, iostat=io_status) parsed_value
            if (io_status == 0) value = parsed_value

        end subroutine s_igr_mhd_read_env_integer
    #:endif
    ...
    impure subroutine s_compute_rhs(q_cons_vf, q_T_sf, q_prim_vf, bc_type, rhs_vf, pb_in, rhs_pb, mv_in, rhs_mv, t_step, &
        ...
        #:if MFC_CASE_OPTIMIZATION and mhd
            call s_mhd_check_cell_admissibility(q_cons_qp%vf, q_prim_qp%vf, t_step, stage)
            call s_igr_mhd_diagnose_cells(q_cons_qp%vf, q_prim_qp%vf, t_step, stage)
            call s_igr_mhd_diagnose_budget_state(q_cons_qp%vf, t_step, stage)
        #:endif
        ...
        do id = 1, num_dims
            ...
            #:if MFC_CASE_OPTIMIZATION and mhd
                call s_mhd_check_face_admissibility(qL_rsx_vf, qR_rsx_vf, id, t_step, stage)
                call s_igr_mhd_diagnose_faces(qL_rsx_vf, qR_rsx_vf, id, t_step, stage)
            #:endif
            ...
            #:if MFC_CASE_OPTIMIZATION and mhd
                call s_igr_mhd_diagnose_flux(flux_n(id)%vf, id, t_step, stage)
            #:endif
            ...
            #:if MFC_CASE_OPTIMIZATION and mhd
                call s_igr_mhd_diagnose_budget_rhs('HYPERBOLIC', q_cons_qp%vf, rhs_vf, id, t_step, stage)
            #:endif
            ...
            if (igr) then
                if (id == 1) then
                    ...
                    #:if MFC_CASE_OPTIMIZATION and igr and mhd
                        if (igr_mhd_debug_enabled) then
                            call s_igr_mhd_diagnose_sigma(t_step, stage, igr_mhd_debug_start, igr_mhd_debug_stop)
                        end if
                    #:endif
                    call nvtxEndRange
                end if
                ...
                #:if MFC_CASE_OPTIMIZATION and igr and mhd
                    call s_igr_mhd_diagnose_budget_rhs('SIGMA', q_cons_qp%vf, rhs_vf, id, t_step, stage)
                #:endif
            end if
            ! END: Additional physics and source terms
        end do
    ...
    #:if MFC_CASE_OPTIMIZATION and mhd
        ! Lightweight, opt-in admissibility checks. These scan every current
        ! RK-stage cell and reconstructed face, but only write on failure.
        subroutine s_mhd_check_cell_admissibility(q_cons_vf, q_prim_vf, t_step, stage)
            ...
            real(wp)                                            :: rho, pres, kinetic, magnetic, margin
                ...
                do kk = 0, n
                    do jj = 0, m
                        ...
                        do ii = eqn_idx%cont%beg, eqn_idx%cont%end
                            rho = rho + q_prim_vf(ii)%sf(jj, kk, ll)
                        end do
                        pres = q_prim_vf(eqn_idx%E)%sf(jj, kk, ll)
                        do ii = 1, sys_size
                            state(ii) = real(q_cons_vf(ii)%sf(jj, kk, ll), wp)
                        end do
        ...
        end subroutine s_mhd_check_cell_admissibility
                ...
                do ll = 0, p
                    do kk = 0, n
                        do jj = 0, m - 1
                            ...
                            do ii = eqn_idx%cont%beg, eqn_idx%cont%end
                                rho_L = rho_L + qR_vf(jj, kk, ll, ii)
                                rho_R = rho_R + qL_vf(jj + 1, kk, ll, ii)
                            end do
                            pres_L = qR_vf(jj, kk, ll, eqn_idx%E)
                            pres_R = qL_vf(jj + 1, kk, ll, eqn_idx%E)
                            call s_mhd_assert_face_admissibility(rho_L, rho_R, pres_L, pres_R, idir, t_step, stage, jj, kk, ll)
                        end do
                    end do
                end do
            else
                do ll = 0, p
                    do kk = 0, n - 1
                        do jj = 0, m
                            ...
                            do ii = eqn_idx%cont%beg, eqn_idx%cont%end
                                rho_L = rho_L + qR_vf(jj, kk, ll, ii)
                                rho_R = rho_R + qL_vf(jj, kk + 1, ll, ii)
                            end do
                            pres_L = qR_vf(jj, kk, ll, eqn_idx%E)
        ...
        end subroutine s_mhd_check_face_admissibility

        subroutine s_mhd_assert_face_admissibility(rho_L, rho_R, pres_L, pres_R, idir, t_step, stage, jj, kk, ll)
        ...
        end subroutine s_mhd_assert_face_admissibility

        ! Opt-in one-rank CPU diagnostics for comparing case-optimized MHD schemes.
        subroutine s_igr_mhd_diagnose_cells(q_cons_vf, q_prim_vf, t_step, stage)
            ...
            real(wp), dimension(sys_size) :: state
            real(wp) :: rho, pres, rho_min, pres_min, kinetic, magnetic, margin, margin_min
            real(wp) :: divb, divb_max, psi_max, target_rho, target_kinetic, target_magnetic, target_margin
            ...
            rho_min = huge(0._wp)
            pres_min = huge(0._wp)
            margin_min = huge(0._wp)
            divb_max = 0._wp
            psi_max = 0._wp
            nonfinite = 0
            rho_j = 0; rho_k = 0; rho_l = 0
            pres_j = 0; pres_k = 0; pres_l = 0
            margin_j = 0; margin_k = 0; margin_l = 0
            divb_j = 0; divb_k = 0; divb_l = 0
                ...
                do kk = 0, n
                    do jj = 0, m
                        ...
                        if (.not. ieee_is_finite(rho) .or. .not. ieee_is_finite(pres) .or. .not. ieee_is_finite(margin) &
                            ...
                            if (rho < rho_min) then
                                rho_min = rho
                                rho_j = jj; rho_k = kk; rho_l = ll
                            end if
                            if (pres < pres_min) then
                                pres_min = pres
                                pres_j = jj; pres_k = kk; pres_l = ll
                            end if
                            if (margin < margin_min) then
                                margin_min = margin
                                margin_j = jj; margin_k = kk; margin_l = ll
                            end if
                            if (abs(divb) > divb_max) then
                                divb_max = abs(divb)
                                divb_j = jj; divb_k = kk; divb_l = ll
                            end if
                        end if
                    end do
                end do
        ...
        subroutine s_igr_mhd_diagnose_faces(qL_vf, qR_vf, idir, t_step, stage)

            real(wp), dimension(idwbuff(1)%beg:,idwbuff(2)%beg:,idwbuff(3)%beg:,1:), intent(in) :: qL_vf, qR_vf
            integer, intent(in) :: idir, t_step, stage
            integer :: ii, jj, kk, ll, ss, nonfinite, bad_j, bad_k, bad_l
            integer, dimension(4, 3) :: min_loc
            real(wp), dimension(4) :: min_value
            real(wp) :: rho_L, rho_R, pres_L, pres_R
            logical :: bad_face
            ...
            min_value = huge(0._wp)
            min_loc = 0
            nonfinite = 0
            bad_face = .false.
            bad_j = 0; bad_k = 0; bad_l = 0
        ...
        subroutine s_igr_mhd_update_face_diag(rho_L, rho_R, pres_L, pres_R, jj, kk, ll, min_value, min_loc, nonfinite, bad_face, &
            ...
            do ii = 1, 4
                if (.not. ieee_is_finite(value(ii))) then
                    nonfinite = nonfinite + 1
        ...
        end subroutine s_igr_mhd_update_face_diag
        ...
        subroutine s_igr_mhd_diagnose_flux(flux_vf, idir, t_step, stage)
            ...
            nonfinite = 0
            max_abs_flux = 0._wp
            first_i = 0; first_j = 0; first_k = 0; first_l = 0
        ...
        subroutine s_igr_mhd_update_flux_diag(value, ii, jj, kk, ll, max_abs_flux, nonfinite, first_i, first_j, first_k, first_l)
            ...
            if (ieee_is_finite(value)) then
                ...
                nonfinite = nonfinite + 1
                if (first_i == 0) then
                    first_i = ii; first_j = jj; first_k = kk; first_l = ll
                end if
            end if

        end subroutine s_igr_mhd_update_flux_diag

        subroutine s_igr_mhd_diagnose_budget_state(q_cons_vf, t_step, stage)

            type(scalar_field), dimension(sys_size), intent(in) :: q_cons_vf
            integer, intent(in)                                 :: t_step, stage
            integer                                             :: ii
            real(wp), dimension(sys_size)                       :: state
            real(wp)                                            :: rho, kinetic, magnetic, margin
        ...
        subroutine s_igr_mhd_diagnose_budget_rhs(label, q_cons_vf, rhs_vf, idir, t_step, stage)

            character(len=*), intent(in)                        :: label
            type(scalar_field), dimension(sys_size), intent(in) :: q_cons_vf, rhs_vf
            integer, intent(in)                                 :: idir, t_step, stage
            integer                                             :: ii
            real(wp), dimension(sys_size)                       :: state, delta, updated
            real(wp)                                            :: rho, kinetic, magnetic, margin
        ...
        subroutine s_igr_mhd_budget_margin(state, rho, kinetic, magnetic, margin)
            ...
            kinetic = 0._wp
            do ii = eqn_idx%mom%beg, eqn_idx%mom%end
                kinetic = kinetic + 0.5_wp*state(ii)**2._wp/rho
            end do

            magnetic = 0._wp
            do ii = eqn_idx%B%beg, eqn_idx%B%end
                magnetic = magnetic + 0.5_wp*state(ii)**2._wp
            end do

            margin = state(eqn_idx%E) - kinetic - magnetic

        end subroutine s_igr_mhd_budget_margin
    #:endif
    ...
    subroutine s_reconstruct_cell_boundary_values(v_vf, vL_x, vR_x, norm_dir)
        ...
        if (igr) then
            #:if MFC_CASE_OPTIMIZATION and mhd
                if (igr_mhd_debug_first_order) then
                    call s_igr_reconstruct_first_order_boundary_values(v_vf, vL_x, vR_x, norm_dir)
                else
        ...
        if (norm_dir == 1) then
            ...
            do i = iv%beg, iv%end
                do l = 0, p
                    do k = 0, n
                        do j = -1, m
                            vL_x(j + 1, k, l, i) = v_vf(i)%sf(j + 1, k, l)
            ...
            do i = iv%beg, iv%end
                do l = 0, p
                    do k = -1, n
                        do j = 0, m
                            vL_x(j, k + 1, l, i) = v_vf(i)%sf(j, k + 1, l)
                            vR_x(j, k, l, i) = v_vf(i)%sf(j, k, l)
                        end do
                    end do
                end do
            ...
            do i = iv%beg, iv%end
                do l = -1, p
                    do k = 0, n
                        do j = 0, m
                            vL_x(j, k, l + 1, i) = v_vf(i)%sf(j, k, l + 1)
                            vR_x(j, k, l, i) = v_vf(i)%sf(j, k, l)
                        end do
                    end do
                end do
    ...
    end subroutine s_igr_reconstruct_first_order_boundary_values
